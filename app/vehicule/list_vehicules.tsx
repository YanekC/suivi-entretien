  ...
  return (
    <main className="items-center justify-center pt-16 pb-4">
      <div className="flex border-b border-gray-200  dark:border-gray-700">
        <h1 className="text-3xl flex-2 m-4">Vos véhicules</h1>
      ...
      <div className="items-center gap-16 min-h-0">
        <div className="w-full space-y-6">
          <ul className="">
            {vehicules.map(({ id, brand, kilometers, model, year }) => (
              <NavLink to={`/vehicules/${id}`} key={id}>
                <li
                  key={id}
                  className="border-b border-gray-200  dark:border-gray-700 p-3 flex flex-row gap-4 max-h-40"
                  onClick={() => {
                    NavLink({ to: `/vehicules/${id}` });
                  }}
                >
                  <img
                    src="/car-placeholder.jpg"
                    alt={`${brand} vehicle`}
                    className="w-3xs h-sm rounded-2xl object-cover"
                  />
